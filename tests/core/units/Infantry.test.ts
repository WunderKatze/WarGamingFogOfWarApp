...
describe("Infantry", () => {
  ...
  it("Recon scales vision by the configured Recon vision multiplier", () => {
    const plain = new Infantry({ id: "1", name: "Plain", teamId: "A", position: at(0, 0) });
    const recon = new Infantry({ id: "2", name: "Recon", teamId: "A", position: at(0, 0), modifiers: ["Recon"] });
  ...
  it("contributes the configured dug-in stealth modifier when dug in", () => {
    const u = new Infantry({ id: "1", name: "Foxhole", teamId: "A", position: at(0, 0), dugIn: true });
  ...
  it("contributes a concealment modifier of 1 when not dug in", () => {
    const u = new Infantry({ id: "1", name: "Standing", teamId: "A", position: at(0, 0) });
    expect(u.dugIn).toBe(false);
    expect(u.getInherentConcealmentModifier()).toBe(1);
  });

  it("dug-in is toggleable", () => {
    const u = new Infantry({ id: "1", name: "Inf", teamId: "A", position: at(0, 0) });
    u.setDugIn(true);
    expect(u.dugIn).toBe(true);
    expect(u.getInherentConcealmentModifier()).toBe(dugInStealthModifier);
    u.setDugIn(false);
  ...
  it("position is mutable through setPosition", () => {
    const u = new Infantry({ id: "1", name: "Mover", teamId: "A", position: at(1, 2) });
    u.setPosition(at(10, 20));
    expect(u.getPosition()).toEqual({ x: 10, y: 20 });
  });

  it("type discriminator is 'Infantry'", () => {
    const u = new Infantry({ id: "1", name: "Pl", teamId: "A", position: at(0, 0) });
